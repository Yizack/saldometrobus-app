...
export const scrapperTarjeta = async (numero: string) => {
  ...
  const output = {
    ...
    tarjeta: {
      numero,
      saldo: Number(cardResponse.servicio.saldo / 100).toFixed(2),
      ...
      movimientos: cardResponse.servicio.listaTransacciones.map(t => ({
        ...
        monto: Number((t.MONTO_ABONO || t.MONTO_DESCUENTO) / 100).toFixed(2),
        saldo_tarjeta: Number(t.saldoTrx / 100).toFixed(2),
        lugar: t.estacion
      }))
    }
  };
